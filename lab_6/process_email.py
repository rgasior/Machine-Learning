...
def process_email(email_contents: str) -> List[int]:
    ...
    email_contents = re.sub('http[s]*://[\S]+', 'httpaddr', email_contents)    #  r'https?://\S+'

    # FIXME: Handle email addresses.
    # Convert all strings with @ in the middle to a 'emailaddr' token.
    email_contents = re.sub('[\S]+@[\S]+', 'emailaddr', email_contents)   #   r'\S+@\S+'

    # FIXME: Handle $ sign
    # Convert all sequences of $ signs to a 'dollar' token.
    email_contents = re.sub('[$]+', 'dollar', email_contents)  #          r'\$+'
